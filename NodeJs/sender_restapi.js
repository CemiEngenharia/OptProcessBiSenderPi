...
*/
    ...
    //                        findAndModifyTags(__postBody['data'], firstDatabase, 0, null, null);
                ...
                else if(slice["1"] == "tagdata")
                {
                    ...
                    {
                        try{
                            len = __postBody["data"].length

                            print("compacted length")
                            print(__postBody["data"].length)

                            const buffer = Buffer.from(__postBody["data"], 'base64');
                            unzip(buffer, (err, buffer) => {
                                ...
                                print("discompacted length")
                                print(buffer.length)

                                print("proporcao de compactacao")
                                print((100-((len / buffer.length) * 100)).toString() + " %")

                                //converte para json a mensagem
                                __postBody = JSON.parse(buffer);
            ...
            //                        addTagData(__postBody, firstDatabase, 0, null, null);
                        ...
                        catch(except)
                        {
                            print(except)
                            res.statusCode = 500;
                            res.write("{'status': 'to parse zip file'}");
                            res.end();
                            print("error to descompact zip")
                            return;
                        }
...
async function addTagData(data, _database, counter=0, res=null, client=null)
{
    ...
    if(res != null)
    {
        //resposta positiva    
        res.statusCode = 500;
        res.write("{'status': 'error to update colections -> debug mode'}");
        res.end();
    }*/
    ...
    */
        ...
        if(!isNaN(element))
        {
            ...
            if(!sessionTags[data['project']].hasOwnProperty(element))
            {
                ...
                await tagsCollection.findOne({"index": parseInt(element)}, function(err, result) {
                        ...
                        // print(result);
                            ...
                            sessionTags[data['project']][parseInt(tagIndex)] = tagAddress;
                        }
                });
            }
            else
            {
                tagAddress = sessionTags[data['project']][element];
            }

            try{
                /*using index as tag name*/
                while(tagAddress.startsWith(".")) tagAddress = tagAddress.substring(1);

                data[tagAddress] = data[element];
